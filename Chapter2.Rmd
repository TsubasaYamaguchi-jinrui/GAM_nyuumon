...
本章では、商業的な漁業が深海(水深800~4865m)の魚の密度に及ぼす影響を調べた @Bailey2009 のデータを用いる。データは2つの期間に分かれており、1979~1989年は商業的な漁業がおこなわれる前(深い水深での漁業が発展している段階)で、1997~2002年は商業的漁業がおこなわれている時期である。商業的漁業は技術的または商業的な理由により水深約1600mまでに限られている。
...
$$
\begin{aligned}
Dens_i &= \alpha + f(Depth_i) +  \epsilon_i (\#eq:gam) \\
\epsilon_i &\sim N(0,\sigma^2)
\end{aligned}
$$
...
## Cross validation  
前節では自由度4で分析を行ったが、自由度はほかの値に設定することも可能である(図\@ref(fig:fig-fit-gam2)。どのようにして最適な自由度を選べばよいだろうか?    
...
これを計算するのは負荷が大きいため、通常はgeneralized cross validation scoreというものを用いることでshortcutを行う。詳細については第\@ref(c3)で解説を行う。データ数が50未満の場合は多重共線性やデータの非独立性によって交差検証の結果に問題が生じることがある。そのため、交差検証の結果をきちんと確認することが必要である。  

## Model validation  
### Normality and homogeneity  
GAMでは重回帰分析のときと同様に残差を抽出し、その正規性や等分散性、独立性、影響のある観察の有無を確認しなければならない。

`gratia`パッケージでは、`appraise`関数でQQプロット、残差 vs 予測値、残差のヒストグラム、実測値 vs 予測値のプロットを作成してくれる(図\@ref(fig:fig-gam-diagnosis))。なお、それぞれのグラフは`qq_plot()`、`residuals_linpred_plot()`、`residuals_hist_plot()`、`observed_fitted_plot`で個別に作成できる。  

この結果から、等分散性や残差の正規性が成立していないことが分かる。    
```{r fig-gam-diagnosis1, fig.dim = c(10,10), fig.cap = "Model diagnosis using `gratia` package."}
appraise(M2_5, type = "response")
```
<br/>  

手動で残差 vs 予測値、残差のヒストグラムは以下のように作成できる(図\@ref(fig:fig-gam-diagnosis2))。
```{r fig-gam-diagnosis2, fig.dim = c(10,5), fig.cap = "Model diagnosis of GAM."}
...
fish_cor <- fish
## coordinateを作成
coordinates(fish_cor) <- ~ Xkm + Ykm

## 算出
vario_M2_5 <- variogram(resid(M2_5) ~ 1, fish_cor)
