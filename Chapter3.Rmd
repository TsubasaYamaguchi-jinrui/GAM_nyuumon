...
$$
||\bf{Y} - \bf{X} \times \bf{\beta}|| + \lambda \times (\bf{\beta^t} \times \bf{D} \times \bf{\beta} - \bf{C})
$$

これを$\bf{\beta}$について解くと、その推定値は以下のようになる。もし$\lambda = 0$ならば(= 何も条件がなければ)、推定値は通常の最小二乗法によるものと同じになる。    
$$
\hat{\bf{\beta}} = (\bf{X^t} \times \bf{X} + \lambda \times \bf{D})^{-1} \times \bf{X^t} \times \bf{y}
$$
...
$$
||\bf{Y} - \bf{X} \times \bf{\beta}||^2 + \lambda \times \int f''(x) dx
$$

@Wood2017 は、この基準で最小になる$f(x)$が3次スプラインであることを証明している。